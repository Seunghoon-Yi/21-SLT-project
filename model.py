...
from Extractor_CNNs.Customized_S3DG_base import S3DG_base
from Extractor_CNNs.C3D_model import C3D
from Extractor_CNNs.CR3D_model import CPlusR3D
from Extractor_CNNs.R3D_model import generate_model
from Extractor_CNNs.Customized_SR3DG import SPlusR3DG
...
class SLT_Transformer(nn.Module):
    def __init__(self,
        ...
        #self.SPlusR3DG = SPlusR3DG(embed_size, device, dropout)
        self.R3D      = generate_model(model_depth=18)
        #self.CPlusR3D = CPlusR3D(embed_size)
    ...
    def forward(self, source, target):
        ...
        #print(source.shape)
        R3D_feature  = self.R3D(source)
        #print("after S+R3d : ", S3D_feature.shape)

        source_mask = self.make_source_mask(R3D_feature)
        target_mask = self.make_target_mask(target)

        # Outputs : glosses and translations
        enc_feature, predict_gloss\
                            = self.Encoder(R3D_feature, source_mask)

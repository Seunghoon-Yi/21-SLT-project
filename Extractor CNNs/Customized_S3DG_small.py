...
class S3DG(nn.Module):
    ...
    def __init__(self, embed_size = 512, device = None, dropout = 0.5, input_channel=3, spatial_squeeze=True):
        ...
        self.features = nn.Sequential(
            ...
            Mixed_4b(),                                                                # (192, 56, 14, 12)
            Mixed_4c(),                                                                # (256, 56, 14, 12)
            nn.MaxPool3d(kernel_size=(2, 2, 2), stride=(2, 2, 2), padding=(0, 0, 0)),  # (256, 56, 7, 6)
            Mixed_5b(),                                                                # (256, 56, 7, 6)
            nn.AdaptiveAvgPool3d(output_size=(56, 2, 2)),                              # (256, 56, 2, 2)
            nn.Dropout3d(p=0.5, inplace=True),                                         # (256, 56, 2, 2)
            nn.Conv3d(256, 128, kernel_size=1, stride=1, bias=True),                   # (128, 56, 2, 2)
        )
    ...
    def forward(self, x):
        ...
        out = self.relu(self.dropout(logits.permute(0,2,1,3,4).reshape(BS, 56, -1)))

...
class Mixed_3c(nn.Module):
    ...
    def forward(self, x):
        ...
        x2 = self.branch2(x)
        x3 = self.branch3(x)
        out = torch.cat((x0, x1, x2, x3), 1).permute(0, 2, 1, 3, 4) + x.permute(0, 2, 1, 3, 4)
        return out
...
class SPlusR3DG(nn.Module):

    def __init__(self, embed_size = 512, device = None, dropout = 0.5, input_channel=3, spatial_squeeze=True):
        super(SPlusR3DG, self).__init__()
        self.features = nn.Sequential(
            ############################# The Stem Network ############################
            BasicConv3d(input_channel, 8, kernel_size=3, stride=1, padding=1),         # (8,  224, 228, 196)
            nn.MaxPool3d(kernel_size=(1, 2, 2), stride=(1, 2, 2), padding=(0, 0, 0)),  # (8,  224, 114, 98)
            BasicConv3d(8, 16, kernel_size=3, stride=1, padding=1),                    # (16, 224, 114, 98)
            nn.MaxPool3d(kernel_size=(2, 2, 2), stride=(2, 2, 2), padding=(0, 0, 0)),  # (16, 112, 57, 49)
            STConv3d(16, 64, kernel_size=3, stride=1, padding=1),                      # (64, 112, 57, 49)
            Permute(),
            nn.LayerNorm([64, 57, 49], elementwise_affine=True),
            ...
            Mixed_3c(),                                                                # (128, 112, 28, 24) //res
            nn.LayerNorm([128, 28, 24], elementwise_affine=True),
            nn.LeakyReLU(negative_slope=0.01, inplace=True),
            Permute(),
            nn.MaxPool3d(kernel_size=(2, 2, 2), stride=(2, 2, 2), padding=(0, 0, 0)),  # (128, 56, 14, 12)
            Mixed_4b(),                                                                # (256, 56, 14, 12)
            Mixed_4c(),                                                                # (256, 56, 14, 12)  //res
            nn.LayerNorm([256, 14, 12], elementwise_affine=True),
            nn.LeakyReLU(negative_slope=0.01, inplace=True),
            Permute(),
            Mixed_4f(),                                                                # (384, 56, 14, 12)
            nn.MaxPool3d(kernel_size=(1, 2, 2), stride=(1, 2, 2), padding=(0, 0, 0)),  # (384, 56, 7, 6)
            Mixed_5b(),                                                                # (384, 56, 7, 6)    //res
            ...
            nn.AdaptiveAvgPool3d(output_size=(56, 2, 2)),                                   # (512, 56, 2, 2)
            nn.Dropout3d(p=0.5, inplace=True),                                              # (512, 56, 2, 2)
            nn.Conv3d(512, embed_size//4, kernel_size=(1,1,1), stride=(1,1,1), bias=True)   # (128, 56, 2, 2)
        )
...
if __name__ == '__main__':
    ...
    model = SPlusR3DG(num_classes=400)
